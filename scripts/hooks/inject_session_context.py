...
def get_plugin_topic_rules(details, templates):
    ...
    if details.get("is_main"):
        ...
        if main_rules and isinstance(main_rules, list):
            ...
            lines.append("  6. 任务派单流转: 寻找专题 -> 没有则按规范创建顶层专题会话 -> sidebus (send_message) 定向发信，划定 Allowlist 物理白名单;")
            lines.append("  7. 复杂度分级调度: Level 1 就地派单，Level 2 标准派单自测，Level 3 专题会话内 Subagent 并行协作;")
            lines.append("  8. 质检与门禁核验: 依据专题会话 (Topic Session) 测试结果与 Evidence 严格验收，输出用户验证指引卡 (参考 references/dispatch-contract.md 与 skills/task-loop/SKILL.md)。")
        ...
        if generic_rules and isinstance(generic_rules, list):
            ...
            lines.append("  2. 专题内子代理协同: 专题会话承接任务后，可按需在专题内拉起子代理 (subagents) 进行多任务拆解协同或直接落地实施;")
            lines.append("  3. 标准执行流程: 承接锁定 -> 边界实施 -> 本地自测 (单测 Exit Code 0) -> 记忆沉淀 (docs/memory/*.md) -> 通过 sidebus 完成标准结构化交付。")
