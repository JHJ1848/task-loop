...
function getPluginTopicRules(details, templates, mainThreadId) {
  ...
  if (details.is_main) {
    if (Array.isArray(pluginRules.main_session) && pluginRules.main_session.length > 0) {
      ...
      lines.push(`  11. 双阶梯看门狗监督机制 (Dual-Stage Watchdog Supervision): 主会话派单后挂载 30s + 120s 监督定时器 (30s 激活探针 + 120s 偏差巡检)。注意定时器冲突管理：设置新定时器前若旧定时器仍在运行，必须先调用 manage_task(Action='kill') 显式销毁旧任务，防范 conflicting early termination condition 报错 (参考 references/dispatch-contract.md)。`);
    }
