...
function getPluginTopicRules(details, templates, mainThreadId) {
  ...
  if (details.is_main) {
    if (Array.isArray(pluginRules.main_session) && pluginRules.main_session.length > 0) {
      ...
      lines.push(`  11. 双阶梯看门狗监督机制 (Dual-Stage Watchdog Supervision): 主会话派单后必须挂载 30s + 120s 监督定时器：① 30s 激活探针 (DurationSeconds=30)：检查专题会话是否已激活启动（步数增加），若未激活则主动干预或提示用户点击 Deep Link 唤醒；② 120s 偏差巡检 (DurationSeconds=120)：检查专题会话执行轨迹与推演逻辑，若发现偏离目标、死循环或逻辑漏洞，立即通过 sidebus 下发修正指示 (参考 references/dispatch-contract.md)。`);
    }
