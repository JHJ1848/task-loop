...
assert.strictEqual(provider.submit({ thread: 't', message: 'p' }, { desktop: () => ({ submitted: false }), sdk: () => ({ submitted: true }) }, { cli: false }).transport, 'sdk');
assert.strictEqual(provider.submit({ thread: 't', message: 'p', dryRun: false, dry_run: true }, { desktop: () => ({ submitted: true }) }, { cli: false }).status, 'PREPARED_ONLY');
...
assert.strictEqual(provider.submit([], {}, { cli: false }).status, 'PREPARED_ONLY');
