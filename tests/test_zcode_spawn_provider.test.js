...
function testApiKeyMode() {
  ...
  // 隔离宿主机真实凭据: 默认 OAuth 通道读取 ~/.zcode/v2/credentials.json，
  // 本机已登录 ZCode 时会让断言 1 误判为已登录。
  const isoEnv = { ZCODE_CLI_CONFIG: cfgPath, ZCODE_CREDENTIALS_PATH: path.join(dir, 'no_credentials.json') };
  try {
    // 1. 空配置 -> 未登录
    let a = provider.authState(isoEnv);
    assert.strictEqual(a.logged_in, false);

    // 2. login-api-key 写入 -> 登录态 true (api-key method)
    const r = provider.loginApiKey({
      key: 'test-key-123', providerId: 'bigmodel', env: isoEnv
    });
