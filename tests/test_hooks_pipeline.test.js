...
function runHooksPipelineTests() {
  ...
  try {
    ...
    // 会注入 CLAUDECODE / CLAUDE_CODE_SESSION_ID，此时 UUID 会话应被正确识别为该宿主会话（见 1.4b）。
    const scrubHostVendorEnv = (env) => {
      const scrubbed = Object.assign({}, env);
      for (const key of [
        'CLAUDECODE', 'CLAUDE_CODE_SESSION_ID', 'CLAUDE_SESSION_ID', 'CLAUDE_CONVERSATION_ID', 'CLAUDE_PROJECT_DIR',
    ...
    const zcodeOutput = execSync(`node "${zcodeScript}"`, {
      input: zcodePayload,
      encoding: 'utf8',
      env: scrubHostVendorEnv(process.env)
    });
    const zcodeResult = JSON.parse(zcodeOutput || '{}');
    assert.strictEqual(Object.keys(zcodeResult).length, 0, 'ZCode adapter should ignore AGY UUID sessions when no host vendor is declared');

    // 1.4b 宿主厂商已声明时，同一 UUID 会话不得再被跳过，须产出注入上下文
    const hostDeclaredOutput = execSync(`node "${zcodeScript}"`, {
      input: zcodePayload,
      encoding: 'utf8',
      env: Object.assign(scrubHostVendorEnv(process.env), { CLAUDECODE: '1' })
    });
    const hostDeclaredResult = JSON.parse(hostDeclaredOutput || '{}');
    assert.strictEqual(
      hostDeclaredResult.hookSpecificOutput && hostDeclaredResult.hookSpecificOutput.hookEventName,
      'UserPromptSubmit',
      'Declared host vendor must make the adapter process UUID sessions instead of skipping them'
    );
    assert.ok(
      hostDeclaredResult.hookSpecificOutput.additionalContext.includes('Unregistered'),
