...
def _write_fixture(home_dir: Path, project_dir: Path):
    proj_posix = project_dir.as_posix()
    munged = munge_project_dir(proj_posix)
    session_dir = home_dir / "projects" / munged
    session_dir.mkdir(parents=True)
    cwd_field = proj_posix
    demo_file_path = (project_dir / "src" / "demo.py").as_posix()
    ...
    lines = [
        ...
        json.dumps({"type": "assistant", "cwd": cwd_field, "timestamp": "2026-08-24T01:00:02.000Z", "message": {"content": [{"type": "tool_use", "name": "Edit", "input": {"file_path": demo_file_path}}]}}),
    ]
...
def test_claude_scanner():
    ...
    try:
        ...
        sessions = scan_claude_sessions(project_dir.as_posix(), custom_claude_home=home_dir.as_posix(), inspect_activity=True)

        assert len(sessions) == 1, f"expected 1 session, got {len(sessions)}"
        s = sessions[0]
        assert s["vendor"] == "claude"
        assert s["session_id"] == SESSION_ID
        assert s["log_path"].replace("\\", "/").endswith(f"{SESSION_ID}.jsonl")
        ...
        assert "src/demo.py" in [f.replace("\\", "/") for f in s["recent_touched_files"]], s["recent_touched_files"]
        ...
        try:
            ...
            wrong_cwd = json.dumps({"type": "user", "cwd": "/elsewhere", "sessionId": SESSION_ID, "timestamp": "2026-08-24T02:00:00.000Z", "message": {"role": "user", "content": "other"}})
            (other_dir / f"{SESSION_ID}.jsonl").write_text(wrong_cwd + "\n", encoding="utf-8")
            assert scan_claude_sessions(project_dir.as_posix(), custom_claude_home=other_home.as_posix()) == []
        finally:
